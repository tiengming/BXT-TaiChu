...
(function(){
    ...
    // 统一的太极螺旋场：所有粒子（包括头部）完全由此场驱动
    function applySpiralField(p) {
        const dx = p.x - cx;
        const dy = p.y - cy;
    ...
    function emitSmoke() {
        if (sim.phase > 2) return;
        const heads = particles.filter(p => p.isHead && p.life > 0);
        if (heads.length < 2) return;
        const headYang = heads.find(p => p.color === COLORS.yang);
        const headYin = heads.find(p => p.color === COLORS.yin);
        if (!headYang || !headYin) return;
        
        for (let i=0; i<CONFIG.SMOKE_PER_FRAME/2; i++) {
            const s1 = new Particle('smoke');
            s1.initSmoke(headYang.x, headYang.y, headYang.vx, headYang.vy, COLORS.yang);
            particles.push(s1);
            const s2 = new Particle('smoke');
            s2.initSmoke(headYin.x, headYin.y, headYin.vx, headYin.vy, COLORS.yin);
            particles.push(s2);
        }
    }

    function drawWhitePoint() {
        if (sim.whitePointSize <= 0.01) return;
        const grad = pCtx.createRadialGradient(cx, cy, 0, cx, cy, sim.whitePointSize * 2.2);
        ...
        pCtx.beginPath();
        pCtx.arc(cx, cy, sim.whitePointSize * 1.8, 0, Math.PI*2);
        pCtx.fill();
        pCtx.shadowBlur = 0;
    }

    function drawFinalText() {
        if (sim.phase !== 3 || sim.textAlpha <= 0.01) return;
    ...
    function render() {
        ...
        if (sim.phase === 2) {
            ...
            if (bangParticles.length < 10 && sim.phase === 2) {
                sim.phase = 3;
                gsap.to(sim, {
                    textAlpha: 1,
    ...
    function startAnim() {
        ...
        const tl = gsap.timeline({
            onUpdate: function() {
                ...
                if (prog < 0.35) phaseHint.innerHTML = '⚲ 化生 · 阴阳交融';
                else if (prog < 0.7) phaseHint.innerHTML = '⚲ 漩涡 · 凝聚归一';
                else if (prog < 0.88) phaseHint.innerHTML = '⚲ 太初 · 光耀大千';
                else phaseHint.innerHTML = '⚲ 真名 · 卜仙堂';
            }
        });
        animationTimeline = tl;

        // 阶段1：角速度从0线性增加到2.5，半径缓慢收缩至0.35
        tl.to(sim, {
            angularSpeed: 2.5,
        ...
        tl.to({}, {
            duration: 0.3,
            onUpdate: function() {
                particles.forEach(p => { if (p.isHead) p.life *= 0.85; });
            }
        }, "-=0.2");
