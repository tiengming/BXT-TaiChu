...
(function(){
    ...
    const COLORS = {
        void: '#0a0a0f', paper: '#EEDEB0', yang: '#FF461F', yin: '#232021',
    ...
    function generateTexture() {
        ...
        for (let i=0; i<d.length; i+=4) {
            ...
            const v = Math.floor(((Math.sin(x*0.02)*Math.cos(y*0.02) + Math.sin(x*0.05+1)*Math.cos(y*0.05)*0.5 + Math.sin(x*0.1)*Math.cos(y*0.1)*0.2)*0.5+0.5)*30);
        ...
        for (let y=0; y<baseH; y+=step) {
            for (let x=0; x<baseW; x+=step) {
                if (img.data[(y*baseW+x)*4] > 128) {
                    const nx = (x/baseW)*2-1, ny = (y/baseH)*2-1;
                    pts.push({ x: cx + nx*Math.min(w,h)*0.38, y: cy + ny*Math.min(w,h)*0.16 });
                }
            }
        }
    ...
    class Particle {
        constructor(x,y,type) {
            ...
            if(type===2){ this.target=null; this.settled=false; }
        }
        update() {
            if(this.type===2 && !this.settled) {
                if(!this.target && textTargetPoints.length) {
        ...
        draw(ctx) {
            ...
            let c=this.type===0?COLORS.yang:(this.type===1?COLORS.cyan:COLORS.gold);
            ctx.fillStyle=c;
            ctx.beginPath();
